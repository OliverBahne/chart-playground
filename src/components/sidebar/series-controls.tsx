...
export function SeriesControls({ config, updateSeries, dispatch }: SeriesControlsProps) {
  ...
  if (chartType === 'combo') {
    return (
      <>
        <LineSeriesSection config={config} updateSeries={updateSeries} dispatch={dispatch} />
        <BarSeriesSection config={config} updateSeries={updateSeries} dispatch={dispatch} />
      </>
    )
  }

  if (chartType === 'line') {
    return <LineSeriesSection config={config} updateSeries={updateSeries} dispatch={dispatch} />
  }

  if (chartType === 'area') {
    return <AreaSeriesSection config={config} updateSeries={updateSeries} dispatch={dispatch} />
  }

  if (chartType === 'bar') {
    return <BarSeriesSection config={config} updateSeries={updateSeries} dispatch={dispatch} />
  }

  return null
}

type SectionProps = Pick<SeriesControlsProps, 'config' | 'updateSeries' | 'dispatch'>
