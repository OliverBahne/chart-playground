...
export function ChartLayoutControls({ config, update }: ChartLayoutControlsProps) {
  ...
  return (
    <FigmaSection title="Chart Layout" defaultOpen={false}>
      ...
      <FigmaNumberInput label="Cat Gap" tooltip={TIPS['Category Gap']} value={cl.barCategoryGap} min={0} max={60} onChange={(v) => update('chartLayout.barCategoryGap', v)} />
      <FigmaNumberInput label="Bar Gap" tooltip={TIPS['Bar Gap']} value={cl.barGap} min={0} max={40} onChange={(v) => update('chartLayout.barGap', v)} />
